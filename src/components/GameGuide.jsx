    ...
    <div className="bg-white dark:bg-gray-800/90 rounded-3xl shadow-xl border border-gray-100 dark:border-gray-700/80 overflow-hidden animate-fade-in">
      ...
      <div className="p-5 lg:p-6 space-y-5 lg:space-y-6 max-h-[75vh] overflow-y-auto">
        {guide.sections.map((section, i) => (
          <div key={i}>
            <h4 className="text-xs lg:text-sm font-bold text-gray-500 dark:text-gray-400 uppercase tracking-wider mb-2.5">
              {section.heading}
            </h4>
            ...
            {section.items && (
              <ul className="space-y-2">
                {section.items.map((item, j) => (
                  <li key={j} className="flex gap-2.5 text-xs lg:text-sm text-gray-600 dark:text-gray-300 leading-relaxed">
                  ...
                  <thead>
                    <tr className="bg-gray-50 dark:bg-gray-700/50">
                      {section.table[0].map((h, k) => (
                        <th key={k} className="px-3 py-2.5 text-left font-bold text-gray-500 dark:text-gray-400 uppercase tracking-wider text-[10px] lg:text-xs">
                          {h}
                        </th>
                      ))}
                    </tr>
