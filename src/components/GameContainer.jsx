  ...
  const statusBar = (
    <div className="flex items-center justify-between">
      <div className="flex items-center gap-3">
        <div className="flex items-center gap-2 px-3 py-1.5 bg-gray-800/90 backdrop-blur-md rounded-xl shadow-sm border border-white/15">
      ...
      <div className="flex gap-1.5">
        <button onClick={onUndo} disabled={!canUndo} title="Undo"
          className="w-10 h-10 flex items-center justify-center rounded-xl bg-gray-800/90 backdrop-blur-md border border-white/10 text-gray-400 shadow-sm disabled:opacity-20 disabled:shadow-none transition-all touch-bounce">
        ...
        <button onClick={() => setShowLog(!showLog)}
          className={`w-10 h-10 flex items-center justify-center rounded-xl border shadow-sm transition-all touch-bounce ${
            showLog
              ...
              : 'bg-white dark:bg-gray-800 border-gray-100 dark:border-gray-700/80 text-gray-400'
          }`}>
          <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    <>
      ...
      <div className="hidden lg:block animate-fade-in">
        ...
        <div className="flex gap-6 items-start">
          ...
          {showLog && (
            <div className="w-[420px] shrink-0 sticky top-4 bg-gray-900/90 backdrop-blur-sm rounded-2xl border border-white/10 flex flex-col max-h-[80vh] animate-slide-in-right">
              <div className="p-5 border-b border-white/10 flex items-center justify-between shrink-0">
                <h3 className="text-sm font-bold text-white/60 uppercase tracking-wider">
                  Lịch sử ({logCount} ván)
                </h3>
                <button onClick={() => setShowLog(false)} className="w-8 h-8 flex items-center justify-center rounded-lg text-white/30 hover:text-white hover:bg-white/10 touch-bounce text-sm">
                  ✕
                </button>
              </div>
      ...
      <div className="lg:hidden space-y-4 animate-fade-in">
        ...
        {showLog && (
          <div className="animate-slide-up">
            <div className="bg-gray-800/95 backdrop-blur-xl rounded-2xl border border-white/15 p-4 shadow-sm">
        ...
        <div className="fixed inset-0 modal-backdrop z-50 flex items-end sm:items-center justify-center animate-fade-in">
          <div className="bg-gray-800/95 backdrop-blur-xl rounded-t-3xl sm:rounded-3xl shadow-2xl p-6 w-full sm:max-w-sm space-y-4 animate-slide-up border-t border-white/15">
            ...
            <div className="text-center">
              ...
              <h3 className="text-lg font-bold text-white mt-2">Kết thúc ván?</h3>
              <p className="text-sm text-gray-400 mt-1">Kết quả sẽ được lưu vào lịch sử</p>
            </div>
            <div className="grid grid-cols-2 gap-3">
              <button onClick={() => setShowEndConfirm(false)} className="py-3 rounded-2xl bg-gray-700 text-gray-300 font-semibold transition-colors touch-bounce">
                Hủy
              </button>
